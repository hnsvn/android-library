...
public class OwnCloudClientManager {
    ...
    private ConcurrentMap<String, HnscloudClient> clientsNewWithKnownUsername = new ConcurrentHashMap<>();

    private ConcurrentMap<String, OwnCloudClient> mClientsWithUnknownUsername = new ConcurrentHashMap<>();
    private ConcurrentMap<String, HnscloudClient> clientsNewWithUnknownUsername = new ConcurrentHashMap<>();
    ...
    public HnscloudClient getHnscloudClientFor(OwnCloudAccount account, Context context)
        ...
        if (account == null) {
            throw new IllegalArgumentException("Cannot get an HnscloudClient for a null account");
        }

        HnscloudClient client = null;
        String accountName = account.getName();
        String sessionName = account.getCredentials() == null ? "" :
                AccountUtils.buildAccountName(account.getBaseUri(), account.getCredentials().getAuthToken());
        ...
        if (client == null) {
            ...
            // no client to reuse - create a new one
            // TODO remove dependency on OwnCloudClientFactory
            client = OwnCloudClientFactory.createHnscloudClient(account.getBaseUri(),
                    userId,
                    credentials,
