...
import com.hnscloud.common.User;
...
public abstract class RemoteOperation<T> implements Runnable {
    ...
    private HnscloudClient clientNew = null;
    ...
    public RemoteOperationResult<T> run(HnscloudClient client) {
     ...
     * @return Result of the operation.
     */
    public RemoteOperationResult<T> executeHnscloudClient(@NonNull Account account, @NonNull Context context) {
        mAccount = account;
        mContext = context.getApplicationContext();
        try {
            OwnCloudAccount ocAccount = new OwnCloudAccount(mAccount, mContext);
            clientNew = OwnCloudClientManagerFactory.getDefaultSingleton().getHnscloudClientFor(ocAccount, mContext);
        } catch (Exception e) {
            Log_OC.e(TAG, "Error while trying to access to " + mAccount.name, e);
            return new RemoteOperationResult<T>(e);
        }
    ...
    /**
     * This is a transitional wrapper around {@link #executeHnscloudClient(Account, Context)}
     * using modern {@link User} interface instead of platform {@link Account}
     */
    public RemoteOperationResult<T> executeHnscloudClient(@NonNull User user, @NonNull Context context) {
        return executeHnscloudClient(user.toPlatformAccount(), context);
    }
     ...
     * @return Result of the operation.
     */
    public RemoteOperationResult<T> execute(@NonNull HnscloudClient client) {
        clientNew = client;
