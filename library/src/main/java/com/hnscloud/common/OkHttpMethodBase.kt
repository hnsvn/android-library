...
abstract class OkHttpMethodBase(
    ...
    /**
     * Execute operation using hnscloud client.
     *
     * @return HTTP return code or [UNKNOWN_STATUS_CODE] in case of network error.
     */
    fun execute(hnscloudClient: HnscloudClient): Int {
        val temp = requestBuilder.url(buildQueryParameter())

        requestHeaders[AUTHORIZATION] = hnscloudClient.credentials
        ...
        try {
            response = hnscloudClient.client.newCall(request).execute()
        } catch (ex: IOException) {
            return UNKNOWN_STATUS_CODE
        }

        return if (hnscloudClient.followRedirects) {
            hnscloudClient.followRedirection(this).lastStatus
        } else {
