...
public class GetCapabilitiesRemoteOperationIT extends AbstractIT {
    ...
    public void testGetRemoteCapabilitiesOperationEtag() {
        ...
        if (capability.getVersion().isNewerOrEqual(OwnCloudVersion.hnscloud_19)) {
     ...
     */
    @Test
    public void testGetRemoteCapabilitiesOperationWithHnscloudClient() {
        // get capabilities
    ...
    @Test
    public void testGetRemoteCapabilitiesOperationEtagWithHnscloudClient() {
        // get capabilities
        RemoteOperationResult result = new GetCapabilitiesRemoteOperation().execute(hnscloudClient);
        assertTrue(result.isSuccess());
        assertTrue(result.getData() != null && result.getData().size() == 1);
        ...
        RemoteOperationResult resultEtag = new GetCapabilitiesRemoteOperation(capability).execute(hnscloudClient);
        assertTrue(resultEtag.isSuccess());
        assertTrue(resultEtag.getData() != null && resultEtag.getData().size() == 1);
        ...
        checkCapability(capability, hnscloudClient.getUserId());
    }
    ...
    private void checkCapability(OCCapability capability, String userId) {
        ...
        // user status
        if (capability.getVersion().isNewerOrEqual(OwnCloudVersion.hnscloud_20)) {
        ...
        // locking
        if (capability.getVersion().isNewerOrEqual(HnscloudVersion.hnscloud_24)) {
            // files_lock app needs to be installed in server for this to work
            assertNotNull(capability.getFilesLockingVersion());
        }

        // groupfolder
        if (capability.getVersion().isNewerOrEqual(HnscloudVersion.hnscloud_27)) {
        ...
        // assistant
        if (capability.getVersion().isNewerOrEqual(HnscloudVersion.hnscloud_28)) {
            if (userId.equals("test")) {
