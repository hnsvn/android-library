...
class StatusIT : AbstractIT() {
    ...
    @Test
    fun getStatus() {
        val result = GetStatusRemoteOperation().run(hnscloudClient)
    ...
    fun setStatus() {
        ...
        val result2 = SetStatusRemoteOperation(StatusType.AWAY).run(hnscloudClient)
        assertTrue("SetStatusRemoteOperation failed: " + result2.logMessage, result2.isSuccess)
    ...
    fun getPredefinedStatuses() {
        val result = GetPredefinedStatusesRemoteOperation().run(hnscloudClient)
        assertTrue(
    ...
    fun clearStatusMessage() {
        val result = ClearStatusMessageRemoteOperation().execute(hnscloudClient)
        assertTrue(
    ...
    fun setPredefinedCustomStatusMessage() {
        ...
        var result = GetPredefinedStatusesRemoteOperation().run(hnscloudClient)
        assertTrue(
            "GetPredefinedStatusesRemoteOperation failed: " + result.logMessage,
            result.isSuccess
        )
        ...
        // verify
        val newResult = GetStatusRemoteOperation().run(hnscloudClient)
    ...
    fun setUserDefinedCustomStatusMessage() {
        ...
        // verify
        result = GetStatusRemoteOperation().run(hnscloudClient)
        assertTrue("GetStatusRemoteOperation failed: " + result.logMessage, result.isSuccess)
