...
class SendCSRRemoteOperationIT : AbstractIT() {
    ...
    fun init() {
        ...
        OwnCloudClientManagerFactory.setUserAgent("Mozilla/5.0 (Android) Hnscloud-android/3.13.0")
    }
    ...
    fun publicKey() {
        ...
        var result = operation.execute(hnscloudClient)

        assertTrue(result.isSuccess)

        // verify public key
        result = GetPublicKeyRemoteOperation(client.userId).execute(hnscloudClient)
        assertTrue(result.isSuccess)
