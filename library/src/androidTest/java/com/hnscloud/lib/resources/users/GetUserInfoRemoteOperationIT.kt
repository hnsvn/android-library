...
class GetUserInfoRemoteOperationIT : AbstractIT() {
    ...
    fun testGetUserNoQuota() {
        hnscloudClient.credentials = Credentials.basic("user1", "user1")
        val userInfoResult = GetUserInfoRemoteOperation().execute(hnscloudClient)
    ...
    fun testGetUser1GbQuota() {
        hnscloudClient.credentials = Credentials.basic("user2", "user2")
        val userInfoResult = GetUserInfoRemoteOperation().execute(hnscloudClient)
        assertTrue(userInfoResult.isSuccess)
        val userInfo = userInfoResult.resultData
    ...
    fun resetCredentials() {
        ...
        hnscloudClient.credentials = Credentials.basic(loginName.orEmpty(), password.orEmpty())
    }
